...
from odoo.addons.odoo_deployment.models.github_repository import github_tokenize
...
class GithubController(http.Controller):
    ...
    @http.route('/odoo_deployment/redirect_url', methods=['POST'], type='http', auth='public', csrf=False)
    def payload(self, payload):
        payload_data = json.loads(payload)
        print (payload_data,'LODDDDdd')
        return 'OK'

    @http.route('/odoo_deployment/payload/<token>', methods=['POST'], type='http', auth='public', csrf=False)
    def payload(self, payload, token):
        ...
        repository_target = request.env['github.repository'].sudo().search(['|', ("name", "=", repository_name), ('github_repository_id', '=', repository_github_id)])
        ...
        if headers_data.get('signature'):
            ...
            if not _consteq(repo_sign, str(headers_data.get('signature'))):
                _logger.info("Github webhook callback recieved request with invalid signature. (GH delivery_id = %s)", headers_data.get('delivery_id'))
                return False

        if repository_target and not repository_target[0].enable_listening:
            _logger.warning('No repository is listening %s', repository_name)
            return 'NO'

        # Create or update target repository
        if not repository_target:
            repository_target = request.env['github.repository'].sudo().create({
        ...
        branch = request.env['github.branch'].sudo().search([('ref', '=', formated_payload.get('payload').get('ref')), ('repository_id', '=', repository_target.id)])
        if not branch:
            repository_target.sync_branches()
            branch = request.env['github.branch'].sudo().search([('ref', '=', formated_payload.get('payload').get('ref')), ('repository_id', '=', repository_target.id)])
            if not branch:
                _logger.info("Sync branch in Repository %s", repository_target.name)
                return False

        for commit in formated_payload.get('payload').get('commits'):
            branch.write({'latest_sha': commit.get('id')})

        if branch.auto_deployment:
        ...
        rendered_template = request.env.ref('odoo_deployment.message_github_notification').render(template_values)
